...
function DriversSection() {
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `http://${import.meta.env.VITE_API_ADDRESS}/users`,
          {
  ...
  const filteredDrivers = (status: string) => {
    ...
    if (status === "available")
      return data.filter((driver: any) => driver.availability === true);
    if (status === "in_use")
      return data.filter((driver: any) => driver.availability === false);
    return [];
  };
  ...
  return (
    <div>
      ...
      <DriverList
        drivers={filteredDrivers(activeTab)}
